...
const SettingsScreen = () => {
  ...
  return (
    ...
    >
      ...
      <View className="bg-blue-600 pt-12 pb-10 px-6 rounded-b-[32px] shadow-sm mb-6">
        <View className="items-center">
          <View className="bg-blue-500/30 p-4 rounded-2xl mb-3 border border-blue-400/20">
            <SettingsIcon size={32} color="white" />
          </View>
          <Text className="text-2xl font-bold text-white mb-1">Configuración</Text>
          <Text className="text-blue-100 text-sm text-center font-medium">
      ...
      <View className="px-5">
        ...
        {/* General Settings */}
        <Card className="bg-white rounded-3xl shadow-sm border border-slate-100 mb-8">
          <CardHeader className="pb-2 pt-5 px-5">
            <View className="flex-row items-center gap-3">
              <View className="bg-blue-50 p-2.5 rounded-xl">
                <SettingsIcon size={20} color="#2563EB" />
              </View>
              <CardTitle className="text-lg font-bold text-slate-800">General</CardTitle>
            </View>
          </CardHeader>
          <CardContent className="px-5 pb-5 pt-2">
            ...
            {settingsOptions.map((option) => {
              ...
              return (
                <View 
                  key={option.title} 
                  className="flex-row items-center justify-between py-4 px-4 rounded-2xl bg-slate-50 border border-slate-100"
                >
                  <View className="flex-row items-center gap-3 flex-1 mr-2">
                    ...
                    <View className="flex-1">
                      <Text className="font-semibold text-slate-800 text-[15px]">{option.title}</Text>
                      <Text className="text-xs text-slate-500 mt-0.5 leading-tight" numberOfLines={1}>
                        {option.description}
                      </Text>
                    </View>
                  </View>
        ...
        <Card className="bg-white rounded-3xl shadow-sm border border-slate-100 mb-8">
          <CardHeader className="pb-2 pt-5 px-5">
            <View className="flex-row items-center gap-3">
              ...
              <Text className="text-lg font-bold text-slate-800">Contactos SOS</Text>
            </View>
          </CardHeader>
          <CardContent className="px-5 pb-5 pt-2">
            <View style={{ gap: 16 }}>
            {emergencyContacts.map((contact) => (
              <View 
                key={contact.number} 
                className="flex-row items-center justify-between p-4 rounded-2xl bg-red-50/50 border border-red-100"
              >
        ...
        <Card className="bg-white rounded-3xl shadow-sm border border-slate-100 mb-8">
          <CardHeader className="pb-2 pt-5 px-5">
            <View className="flex-row items-center gap-3">
              ...
              <Text className="text-lg font-bold text-slate-800">Información</Text>
            </View>
          </CardHeader>
          <CardContent className="px-5 pb-5 pt-2">
            <View style={{ gap: 16 }}> {/* Espaciado entre elementos */}
              
              <View className="flex-row justify-between items-center py-4 px-4 rounded-2xl bg-slate-50 border border-slate-100">
                <Text className="text-sm font-semibold text-slate-600">Versión</Text>
                <Badge className="bg-blue-100 text-blue-700 border-0">1.0.0</Badge>
              </View>
              
              <View className="flex-row justify-between items-center py-4 px-4 rounded-2xl bg-slate-50 border border-slate-100">
                <Text className="text-sm font-semibold text-slate-600">Modo</Text>
                <Badge className="bg-emerald-100 text-emerald-700 border-0">Offline</Badge>
              </View>
              
              <View className="flex-row justify-between items-center py-4 px-4 rounded-2xl bg-slate-50 border border-slate-100">
                <Text className="text-sm font-semibold text-slate-600">Guías</Text>
                <Badge className="bg-blue-100 text-blue-700 border-0">10 Disponibles</Badge>
              </View>
              
              <View className="flex-row justify-between items-center py-4 px-4 rounded-2xl bg-slate-50 border border-slate-100">
                <Text className="text-sm font-semibold text-slate-600">Actualizado</Text>
                <Text className="text-xs font-bold text-slate-400">20 AGO 2025</Text>
              </View>
        ...
        {/* Credits */}
        <Card className="bg-blue-50/50 rounded-3xl border border-blue-100 overflow-hidden mb-8">
          ...
          <CardHeader className="pb-2 pt-5 px-5">
            <View className="flex-row items-center gap-3">
              ...
              <Text className="text-lg font-bold text-slate-800">Sobre FirstAId</Text>
            </View>
          </CardHeader>
          <CardContent className="px-5 pb-5 pt-2">
            <Text className="text-sm text-slate-600 leading-relaxed mb-5">
              FirstAId es tu compañero confiable para emergencias médicas offline.
            </Text>
            
            <View style={{ gap: 12 }}>
              <View className="bg-white/80 p-4 rounded-xl flex-row items-center gap-3 border border-blue-100/50">
                <Users size={16} color="#3B82F6" />
                <Text className="text-xs text-slate-700 font-medium">Equipo FirstAId</Text>
              </View>
              <View className="bg-white/80 p-4 rounded-xl flex-row items-center gap-3 border border-blue-100/50">
                <Shield size={16} color="#10B981" />
                <Text className="text-xs text-slate-700 font-medium">Validado por médicos</Text>
              </View>
            </View>
          </CardContent>
        </Card>
